...
import { getPricesAndCalculateValues } from "/js/priceFetch/priceFetcher.js";
...
function updateDatalist(addresses) {
  ...
  addressDatalist.innerHTML = addresses
    .map((addr) => `<option value="${addr}"></option>`)
    .join("");
...
async function fetchAssets() {
  ...
  try {
    ...
    if (response.status !== 200 || data.error) {
      throw new Error(
        data.details || data.error || "The server returned an error.",
      );
    }
    console.log("[main] data", data);

    const { assets: assetsWithValues, totalUsdValue } =
      await getPricesAndCalculateValues(data.assets || []);
    console.log(
      "[main] assetsWithValues",
      assetsWithValues,
      "[main] totalUsdValue",
      totalUsdValue,
    );
...
function buildSidebar(assets) {
  ...
  const existingProtocols = new Set(
    Array.from(sidebarMenu.querySelectorAll(".sidebar-item")).map(
      (item) => item.dataset.protocol,
    ),
  );
  ...
  sidebarMenu.querySelectorAll(".sidebar-item").forEach((item) => {
    item.addEventListener("click", (e) => {
      ...
      if (protocol === "all") {
        resultsContainer
          .querySelectorAll(
            ".dapp-group-default, .dapp-group-wallet, .dapp-group-xflows, .dapp-group-xstake",
          )
          .forEach((group) => {
...
function init() {
  ...
  document
    .querySelector('.sidebar-item[data-protocol="all"]')
