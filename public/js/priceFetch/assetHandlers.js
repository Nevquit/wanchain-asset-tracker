...
function calculateComplexPositionAssetValue(asset, pricesMap) {
    ...
    if (asset.extra?.primary_assets) {
        asset.extra.primary_assets = asset.extra.primary_assets.map(subAsset => {
            const subAssetWithAddress = { ...subAsset, address: subAsset.address || subAsset.asset_ca };
            const { usdPrice, usdValue } = getAssetPriceAndValue(subAssetWithAddress, pricesMap);
...
function calculateXStakeFarmingAssetValue(asset, pricesMap) {
    ...
    const { usdPrice: stakedPrice, usdValue: stakedValue } = getAssetPriceAndValue(stakedAssetTemp, pricesMap);
    ...
    if (asset.extra && asset.extra.reward) {
        ...
        const rewardAssetTemp = { address: rewardAsset.asset_ca, amount: rewardAsset.amount };
        const { usdPrice, usdValue } = getAssetPriceAndValue(rewardAssetTemp, pricesMap);
