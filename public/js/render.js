...
window.copyToClipboard = function (text, element) {
  if (navigator.clipboard && navigator.clipboard.writeText) {
    ...
    alert(
      "Your browser does not support auto-copy function. Please copy manually: " +
        text,
    );
  }
};
...
export function renderResults(
  ...
  if (assets.length > 0) {
    ...
    totalDiv.style.cssText =
      "font-size: 1.8em; font-weight: bold; margin-bottom: 25px; padding: 15px; border-bottom: 3px solid #007bff; background-color: #f8f9fa; border-radius: 5px; text-align: center;";
  ...
  if (assets.length === 0 && failedProtocols.length === 0) {
    resultsContainer.innerHTML +=
      '<p style="text-align: center; color: #7F8C8D; margin-top: 30px;">No assets found for this address across all protocols.</p>';
    return;
  }
