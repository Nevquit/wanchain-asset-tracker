  ...
  try {
    ...
    const fetchPromises = protocolFiles.map(async (file) => {
      ...
      try {
        ...
        console.log(
          `   - [${protocolName}] Found function: ${exportedFunctionName}`,
        );
        ...
        // --- 核心调试日志 ---
        console.log(
          `   - [${protocolName}] Raw output received:`,
        ...
        if (Array.isArray(assets)) {
          console.log(
            `   - [${protocolName}] Success. Found ${assets.length} assets.`,
          );
          return { protocolName, assets };
        } else {
          console.warn(
            `   - [${protocolName}] WARNING: Did not return an array. Assuming failure.`,
          );
    ...
    console.error(
      "--- [Orchestrator] CRITICAL ERROR: Failed to read protocols directory ---",
      error.message,
    );
    return {
      assets: [],
