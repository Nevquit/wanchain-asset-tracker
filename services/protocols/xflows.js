...
function getV3Amounts(
  ...
  if (activeTick === tickUpper) {
    amount0 =
      (liquidity * (sqrtRatioB - sqrtRatioA) * Q96) / (sqrtRatioA * sqrtRatioB);
    amount1 = 0n;
    ...
    amount0 =
      (liquidity * (sqrtRatioB - sqrtRatioC) * Q96) / (sqrtRatioC * sqrtRatioB);
    amount1 = (liquidity * (sqrtRatioC - sqrtRatioA)) / Q96;
  }
...
async function getTokenInfo(tokenAddr) {
  ...
  try {
    ...
    // 捕获失败，返回 'UNKNOWN'
    console.error(
      `[ERROR] Failed to fetch token info for ${tokenAddr}:`,
      e.message,
    );
    ...
    for (let i = 0; i < positionCount; i++) {
      ...
      try {
        ...
        if (poolAddress && poolAddress !== ZeroAddress) {
          ...
          console.log(
            `[${Dapp}] Successfully fetched real-time Tick: ${currentTick} and calculated theoretical fees for #${tokenId.toString()}.`,
          );
        } else {
          console.warn(
            `[${Dapp}] Pool not found for position #${tokenId.toString()}. Using recorded fees.`,
          );
        }
      } catch (e) {
        // 4. 回退：如果实时调用失败，则回退到仅使用已记录的 fees (tokensOwed)
        console.warn(
          `[${Dapp}] Failed full fee calculation for #${tokenId.toString()}. Falling back to recorded fees. Error: ${e.message}`,
        );
