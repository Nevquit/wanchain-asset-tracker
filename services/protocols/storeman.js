...
function initializeApiClient() {
  return new IWAN(IWAN_CONFIG.API_KEY, IWAN_CONFIG.SECRET_KEY, {
    url: IWAN_CONFIG.URL,
...
export async function getStoremanAssets(userAddr) {
  ...
  try {
    ...
    const { deposit: totalDeposit, incentive: totalIncentive } =
      accumulateStoremanValues(delegationResults);

    // 2. 查询质押 (Stake/Validator) 资产
    const stakeResults = await apiClient.getStoremanStakeInfo({
      address: [userAddr],
    });
    const { deposit: totalStake, incentive: totalStakeIncentive } =
      accumulateStoremanValues(stakeResults);
