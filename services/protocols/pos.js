...
export async function getPoSAssets(userAddr) {
  ...
  if (!userAddr || userAddr.length < 40) {
    // 简单检查长度
    console.error(
      "[ERROR] PoS asset check failed: Invalid or missing user address provided to getPoSAssets.",
    );
    return [];
  }
  ...
  try {
    ...
    const delegationResults = await apiClient.getDelegatorStakeInfo(
      "WAN",
      userAddr,
    );
    ...
    const errorMessage =
      e && e.message
        ...
        : e
          ? e.toString()
          : "Unknown error during PoS API call.";
